 ...
 * <pre>
{
	...
	To:	// 消息接收人，接收人可以是单个对象、也可以多个（数组）。因为微信不能用一个唯一 ID (加密且每次登录会改变) 来指定接收人，
		// 所以需要通过指定 Account、Alias、RemarkName、NickName、DisplayName 几个名称中的一个或多个的方式来定位该接收人。
		[
			{	// 这几个字段，至少需要指定其中的一个。
				Account: "明文帐号，或通配符帐号",	// 明文帐号： 是在手机端打开一个联系人聊天窗口，然后在网页端获取到的明文，诸如： wxid_XXXX (比较常见的个人帐号)、 gh_XXX (公众号帐号)、 NNNNNN@chatroom (群聊帐号)、还有一些用户自定义的帐号（早期注册的？）
